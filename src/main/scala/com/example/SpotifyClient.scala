...
class SpotifyClient(val authToken: String ){
  ...
  object playlist {
    ...
    def getUsersPlaylists(userId: String): spotifyApiModels.Page[spotifyApiModels.PlaylistSimple] = {
      ...
      try {
        Await.result(Unmarshal(response.entity).to[spotifyApiModels.Page[spotifyApiModels.PlaylistSimple]], 2.second)
      }
    }
    ...
    def getPlaylist(playlistId: String): spotifyApiModels.Playlist ={
      ...
      val response = Await.result(Http().singleRequest(request), 2.second)
      try {
        Await.result(Unmarshal(response.entity).to[spotifyApiModels.Playlist], 2.second)
      }
    }
  }
  ...
  object track {
    ...
    def getTrack(trackId: String):spotifyApiModels.Track =  {
      ...
      val response = Await.result(Http().singleRequest(request), 2.second)
      try {
        Await.result(Unmarshal(response.entity).to[spotifyApiModels.Track], 2.second)
      }
    }
    ...
    def getTracksAudioFeatures(tracksIds: List[String]): spotifyApiModels.listAudioFeatures = {
      ...
      val response = Await.result(Http().singleRequest(request), 2.second)
      try{
        Await.result(Unmarshal(response.entity).to[spotifyApiModels.listAudioFeatures], 2.second)
      }
    }
    ...
    def getTrackAudioFeatures(trackId: String): spotifyApiModels.AudioFeatures = {
      ...
      val response = Await.result(Http().singleRequest(request), 2.second)
      try{
        Await.result(Unmarshal(response.entity).to[spotifyApiModels.AudioFeatures], 2.second)
      }
